        ...
        <div className="px-4 sm:px-0">
          ...
          <div className="bg-white border border-gray-200 rounded-lg overflow-hidden shadow-sm">
            <div className="bg-gray-50 px-4 py-3 border-b border-gray-200">
               <span className="text-sm font-semibold text-gray-900">{repository.issues.length} Open</span>
            </div>
            <ul className="divide-y divide-gray-200">
              {repository.issues.length > 0 ? (
                repository.issues.map((issue: any) => (
                  <li key={issue.id} className="hover:bg-gray-50">
                    <div className="px-4 py-4 sm:px-6">
                      <div className="flex items-center justify-between">
                        <Link href={`/repos/${owner}/${repo}/issues/${issue.id}`} className="text-sm font-bold text-gray-900 hover:text-indigo-600">
                          {issue.title}
                        </Link>
                      </div>
                      <div className="mt-2 flex items-center text-xs text-gray-500">
                        <span>#{issue.id.slice(-4)}</span>
                        <span className="mx-1">opened on {issue.createdAt.toLocaleDateString()}</span>
                        <span className="mx-1">by {issue.author.name || issue.author.email}</span>
                      </div>
                    </div>
                  </li>
                ))
